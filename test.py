...
try:
    ...
    optimum = opt.minimize(f,x0,constraints=constraints,method='grg',file_tag="_test",max_processes=8,termination_tol=1e-9)
    print("Optimum value: {0}".format(optimum.f))
    ...
    for calls in optimum.cstr_calls:
        cstr_calls += calls
    print("Constraint calls: {0}".format(cstr_calls))
    message.append("Passed 3 variable, 4 inequality constraint, central differencing, 8 process GRG test.\n")
except:
    message.append("Failed 3 variable, 4 inequality constraint, central differencing, 8 process GRG test.\n")
    message.append("Unexpected error: {0}\n".format(sys.exc_info()))
...
try:
    ...
    message.append("Unexpected error: {0}".format(sys.exc_info()))
